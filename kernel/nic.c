...
static void init_tx() {
    ...
    /* TDT */
    set_nic_register(0x3818, (unsigned int) 1);
    /* TCTL */
...
void init_nic(unsigned int nic_address) {
    ...
    /* IMC */
    set_nic_register(0x00d8, 0xffffffff);
...
unsigned char send_frame(void *buffer, unsigned short len) {
    for (unsigned int i=0; i<len; i++) {
        ...
        /* length */
        tx_descriptors[(0+i)%TX_DESCRIPTORS_NUM].legacy = tx_descriptors[(0+i)%TX_DESCRIPTORS_NUM].legacy | 0x8;
        /* TDT */
        set_nic_register(0x3818, (unsigned int) (2+i)%TX_DESCRIPTORS_NUM);
